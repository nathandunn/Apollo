...
//        dataGrid.redraw();
    ...
    private static TreeItem buildTree(List<String> items) {
        String rootItem = items.get(0);
        // clone but the first 1
        for (int i = 0; i < dataGrid.getItemCount(); i++) {
            if (dataGrid.getItem(i).getWidget().getElement().getId().startsWith(rootItem)) {
                TreeItem treeItem = dataGrid.getItem(i);
                return makeTreeItem(treeItem, items, 1);
            }
        }
        // else we need to make it
        Label rootLabel = new Label(rootItem);
        rootLabel.getElement().setId(rootItem);
        TreeItem treeItem = new TreeItem(rootLabel);
        dataGrid.addItem(treeItem);
        return makeTreeItem(treeItem, items, 1);
    }

    private static String generateForListAndIndex(List<String> items, int index) {
        String string = "";
    ...
    private static TreeItem makeTreeItem(TreeItem treeItem, List<String> items, int index) {
        if(index > items.size()-1){
            GWT.log("too deep: "+ index + " vs " + items.size());
            return treeItem;
        }
        String key = generateForListAndIndex(items, index);
        GWT.log("key: "+key  + " for index: "+ index);
        for (int i = 0; i < treeItem.getChildCount(); i++) {
            TreeItem childItem = treeItem.getChild(i);
            String childId = childItem.getElement().getId();
            GWT.log("childId: "+childId);
            if (key.equals(childId)) {
                GWT.log("found child: "+childId);
                return childItem;
            } else if (childId.contains(key)) {
                GWT.log("found parent, going to child: "+childId);
                return makeTreeItem(childItem, items, index + 1);
            }
        }

        // if we don't find anything, then we need to build something and then keep going
        GWT.log("adding new item: "+items.get(index));
        TreeItem newTreeItem = new TreeItem();
        newTreeItem.setHTML(items.get(index));
        newTreeItem.getElement().setId(key);
        treeItem.addItem(newTreeItem);
        return makeTreeItem(newTreeItem,items,index+1);
    }
    ...
    private static void renderFiltered() {
        ...
        for (TrackInfo trackInfo : trackInfoList) {
            ...
            List<String> categoryList = trackInfo.getCategoryAsList();
            buildTree(categoryList);
            // get index
//            dataGrid.getI

            // put categories in the tree
        }
